...
class SmoothDeltaController5DOF(Node):
    def __init__(self):
        ...
        self.joint_pub_real = self.create_publisher(
            JointTrajectory, "/delta/joint_commands", 10
        )
        ...
        self.pose_sub = self.create_subscription(
            Pose, "/delta/target_pose", self.target_callback, 10
        )
        self.cart_sub = self.create_subscription(
            JointTrajectory, "/delta/cartesian_trajectory", self.trajectory_callback, 10
        )
        self.speed_sub = self.create_subscription(
            Twist, "/delta/speed_params", self.speed_callback, 10
        )
        self.offset_sub = self.create_subscription(
            Float64MultiArray, "/delta/offsets", self.offset_callback, 10
        )

        self.timer = self.create_timer(self.dt, self.control_loop)
        self.get_logger().info(
            "5DOF TCP Controller Started @ 100Hz (Tilt Compensation Ready)"
        )
    ...
    def control_loop(self):
        if self.is_moving:
            dx, dy, dz = (
                ...
                self.target_pos[2] - self.current_pos[2],
            )
            dist = math.sqrt(dx * dx + dy * dy + dz * dz)
            step_dist, step_ang = (
                self.linear_speed * self.dt,
                self.angular_speed * self.dt,
            )
            ...
            tilt_err, spin_err = (
                ...
                self.target_spin - self.current_spin,
            )
            ...
            if (
                dist <= step_dist
                and abs(tilt_err) <= step_ang
                and abs(spin_err) <= step_ang
            ):
                self.is_moving = False
        ...
        try:
            ...
            if tilt_comp:
                ...
                wrist_y = self.current_pos[1] + (
                    rotating_length * math.sin(self.current_tilt)
                )
                wrist_z = (
                    self.current_pos[2]
                    - static_drop
                    - (rotating_length * math.cos(self.current_tilt))
                )
            else:
            ...
            # 6. Broadcast TF for visualization
            t = TransformStamped()
            t.header.stamp = self.get_clock().now().to_msg()
            t.header.frame_id = "delta_robot/world_link"
            t.child_frame_id = "delta_robot/commanded_end_effector_pin"
            
            # Using current_pos (TCP) for the commanded TF
            # Applying -90deg CW rotation to align User Frame with Robot/SDF Frame
            t.transform.translation.x = float(self.current_pos[1])
            t.transform.translation.y = -float(self.current_pos[0])
            t.transform.translation.z = float(self.current_pos[2])
            
            # Convert tilt/spin to quaternion
            # Tilt is roll, Spin is yaw
            # Rotate orientation by -90deg CW (-pi/2 yaw)
            target_yaw = self.current_spin - (math.pi / 2.0)
            cy = math.cos(target_yaw * 0.5)
            sy = math.sin(target_yaw * 0.5)
            cp = math.cos(0.0)
            sp = math.sin(0.0)
            cr = math.cos(self.current_tilt * 0.5)
            sr = math.sin(self.current_tilt * 0.5)
