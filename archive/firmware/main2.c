/**
 * @file main2.c (ARCHIVED)
 * @brief Mutex-Protected Buffer + S-Curve Interpolation
 *
 * CHANGES FROM main1.c:
 * - Added SemaphoreHandle_t buffer_mutex for thread safety
 * - 100Hz Motion Loop (2 sub-steps per packet)
 * - S-Curve phase interpolation: phase^2 * (3 - 2*phase)
 * - WiFi Power Save OFF for low latency
 * - Buffer Size 64, Startup Count 10
 *
 * ISSUES: S-curve still causes stop-start jerk.
 */
...
#define MOTION_FREQ_HZ                                                         \
  100 // 50Hz Packet * 2 = 100Hz Motion Loop (Min 10ms tick support)
  ...
  while (1) {
    ...
    if (!active) {
      ...
      for (int i = 0; i < NUM_SERVOS; i++) {
        float ideal_angle =
            start_pt.angles[i] +
            (end_pt.angles[i] - start_pt.angles[i]) * s_curve_phase;
