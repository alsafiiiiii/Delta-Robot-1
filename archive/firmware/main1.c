/**
 * @file main1.c (ARCHIVED)
 * @brief 3-Motor Simplified Driver (Arms Only)
 *
 * CHANGES FROM main.c:
 * - Reduced to 3 servos (GPIOs 2, 4, 5)
 * - Single MCPWM Group 0 (simplified)
 * - INTERP_STEP = 1.0 (No sub-stepping)
 * - 3MHz Timer Resolution for higher PWM precision
 *
 * ISSUES: No sub-stepping = jerky motion between packets.
 */
...
typedef struct {
  float angles[5];
} delta_packet_t; // Keep struct size same for compatibility
  ...
  while (1) {
    ...
    if (!active) {
      ...
      for (int i = 0; i < NUM_SERVOS; i++) {
        ...
        float ideal_angle =
            start_pt.angles[i] +
            (end_pt.angles[i] - start_pt.angles[i]) * linear_phase;
