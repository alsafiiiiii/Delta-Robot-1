    ...
    while(1) {
        ...
        if (!active) {
            ...
            for (int i = 0; i < NUM_SERVOS; i++) {
                ...
                float ideal_angle = start_pt.angles[i] + (end_pt.angles[i] - start_pt.angles[i]) * phase;
                
                // 2. APPLY DEADBAND FILTER (The "Jitter Killer")
                // Only send a new PWM pulse if the change is significant (> 0.4 degrees)
...
void app_main(void) {
    ...
    mcpwm_timer_config_t t_cfg = { .clk_src=MCPWM_TIMER_CLK_SRC_DEFAULT, .resolution_hz=1000000, .period_ticks=20000, .count_mode=MCPWM_TIMER_COUNT_MODE_UP };
    t_cfg.group_id=0; ESP_ERROR_CHECK(mcpwm_new_timer(&t_cfg, &t0));
    t_cfg.group_id=1; ESP_ERROR_CHECK(mcpwm_new_timer(&t_cfg, &t1));

    for (int i=0; i<NUM_SERVOS; i++) {
        int gid = (i<3)?0:1; mcpwm_timer_handle_t timer = (i<3)?t0:t1;
        mcpwm_oper_handle_t oper; mcpwm_operator_config_t o_cfg = { .group_id=gid };
        mcpwm_new_operator(&o_cfg, &oper); mcpwm_operator_connect_timer(oper, timer);
        mcpwm_cmpr_handle_t cmpr; mcpwm_comparator_config_t c_cfg = { .flags.update_cmp_on_tez=true };
        mcpwm_new_comparator(oper, &c_cfg, &cmpr); comparators[i]=cmpr;
        mcpwm_gen_handle_t gen; mcpwm_generator_config_t g_cfg = { .gen_gpio_num=SERVO_GPIOS[i] };
    ...
    mcpwm_timer_enable(t0); mcpwm_timer_start_stop(t0, MCPWM_TIMER_START_NO_STOP);
    mcpwm_timer_enable(t1); mcpwm_timer_start_stop(t1, MCPWM_TIMER_START_NO_STOP);
