...
class InteractiveDelta:
    ...
    def solve_ik(self, x, y, z):
        ...
        if not valid_global: return None
...
gs = gridspec.GridSpec(2, 3, width_ratios=[1.5, 1, 0.5], height_ratios=[1, 1], wspace=0.2, hspace=0.2)
...
def init_plot():
    ...
    proj_offset_line, = ax_3d.plot([], [], [], color='purple', linestyle='--', lw=2, zorder=10)
...
def draw_projection_plane(arm_idx):
    global proj_plane
    if proj_plane: proj_plane.remove()
...
def update_equations(state, i, x, y):
    ...
    ax_eq.text(0.05, 0.95, s, transform=ax_eq.transAxes, verticalalignment='top', 
               fontsize=11, color=text_color, family='monospace', linespacing=1.4)
...
def update_plot(val=None):
    ...
    # Clear 3D labels
    for txt in text_labels_3d: txt.remove()
    text_labels_3d = []
    ...
    if elbows is None:
        ...
        for i in range(3):
            ...
            lines_bicep[i].set_data_3d([bx, elbows[i,0]], [by, elbows[i,1]], [0, elbows[i,2]])
            lines_forearm[i].set_data_3d([elbows[i,0], px], [elbows[i,1], py], [elbows[i,2], z])
    ...
    t2 = ax_3d.text(mid_x, mid_y, z+0.01, "y'", color='purple', fontsize=10, fontweight='bold')
...
def submit_x(text):
    try: current_pos[0] = float(text); update_plot()
    except: pass
def submit_y(text):
    try: current_pos[1] = float(text); update_plot()
    except: pass
def submit_z(text):
    try: current_pos[2] = float(text); update_plot()
    except: pass
...
fig.text(0.85, 0.25, instr_text, fontsize=9, ha='center', va='center', 
